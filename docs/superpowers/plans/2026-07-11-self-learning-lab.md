...
class SelfLearningLabContractTests(unittest.TestCase):
    ...
    def test_labs_remain_outside_batch_install_and_active_audit(self) -> None:
        install_script = (REPO_ROOT / "install.sh").read_text(encoding="utf-8")
        audit_script = (REPO_ROOT / "scripts" / "audit_skills.py").read_text(
            encoding="utf-8"
        )
        self.assertNotIn('"$REPO_DIR/labs"', install_script)
        self.assertIn('("skills", "plugins", "harness")', audit_script)
...
```bash
python3 -m unittest tests.test_self_learning_lab_contract -v
```
...
python3 "$HOME/.codex/skills/.system/skill-creator/scripts/init_skill.py" \
  ...
  --interface 'short_description=从外部素材自主学习并生成可验证产物' \
...
- 复杂、长时间、无人值守或需要断点恢复的任务，先生成本轮薄 Workflow。
...
固定：用户目标、指定能力、真实依赖、授权边界、人工检查门、验收条件。

动态：具体步骤、工具候选、步骤数量、并行与顺序、产物组合，以及新证据出现后的调整。
...
把 `yaml-contracts.md` 的 `workflow.yaml` 章节改为：只有固定阶段已经被真实 SOP 验证、且存在确定性机器解析或跨进程交接需求时才使用；开放任务的本轮薄 Workflow 默认使用 Agent 计划或 Markdown，不先建 YAML 状态机。
...
```bash
python3 -m unittest \
  tests.test_self_learning_lab_contract.SelfLearningLabContractTests.test_philosophy_links_runtime_workflows \
  -v
```
...
- [ ] **Step 2: 先增加旧持久 marker 迁移测试**

在复制后的 `test_init_safety.py` 增加一个测试：写入旧的根级与嵌套 `.gitignore` marker，运行 `start` 后断言旧 marker 被移除、新 marker 只出现一次、用户规则仍保留，第二次运行保持字节幂等。
...
Expected: 新测试 FAIL，因为脚本尚不识别旧 block pair。
...
- [ ] **Step 4: 迁移持久 marker 与故障注入环境变量**

在 `init_run.py` 中：

- 当前 marker 改为 `self-learning-hyperframes`；
- 保留旧 root/nested marker pair 作为明确的 legacy 输入；
...
```bash
python3 -m unittest \
  tests.test_self_learning_lab_contract.SelfLearningLabContractTests.test_main_skill_keeps_workflow_dynamic \
  tests.test_self_learning_lab_contract.SelfLearningLabContractTests.test_public_surfaces_use_only_new_names \
  tests.test_self_learning_lab_contract.SelfLearningLabContractTests.test_readmes_classify_self_learning_as_lab \
  tests.test_self_learning_lab_contract.SelfLearningLabContractTests.test_labs_remain_outside_batch_install_and_active_audit \
  -v
```
...
```bash
j-skills uninstall tutorial-to-hyperframes-demo -g -y --json
j-skills link --unlink tutorial-to-hyperframes-demo --json
```

Expected: 旧的 Claude/Codex 安装与 link registry 条目均被删除，新名称仍然可用。
...
```bash
if test -f skills/tutorial-to-hyperframes-demo/experience.local.md; then
  test -f labs/self-learning/self-learning-hyperframes/experience.local.md
fi
...
```bash
python3 \
  labs/self-learning/self-learning-hyperframes/tests/test_run_contract.py \
  ValidateRunTests.test_complete_semantic_run_passes \
  -v
python3 \
  labs/self-learning/self-learning-hyperframes/tests/test_validator_truth.py \
  TruthBoundValidatorTests.test_strict_truth_bound_run_passes \
  -v
```

Expected: 两项 PASS。

- [ ] **Step 2: 在临时目录 check/render 一个已有成功 Demo**
...
Expected: `check`、`render`、MP4 存在性与 `ffprobe` 全部成功；不修改真实 Demo 仓库。
...
- [ ] **Step 5: 用新鲜子 Agent 前向测试总入口**

分别给新鲜子 Agent 两个原始任务，不提供设计结论：
...
Expected: 第一个任务可以直接执行而不制造固定 Workflow；第二个任务保留指定工具与人工检查门，同时自行决定其余路径。若输出出现固定通用阶段、把 Self Learning 解释为模型训练，或忽略人工门，先修 `SKILL.md` 再重测。
