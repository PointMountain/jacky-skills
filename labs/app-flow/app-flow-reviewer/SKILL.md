...
- 在当前 scope 内定义决策、acceptance 与可证伪的 must-pass facts；不引入固定技术栈、通用分数或**跨任务固定门槛**（区别于下条的"关键转换点各评一次"——那是"何时评审"的触发点，不是固定评分/阶段）。
- 不在每个琐碎能力之后自动跟随；但在**改变承诺的关键转换点**（spec / 设计 / 构建 / 发布前）各应触发一次独立评审，对应四个 rubric——这不同于"整条链路只评一次"（那样会漏掉设计/交互层问题）。跳过某个 gate 要显式说明理由。
