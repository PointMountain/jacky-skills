...
description: "编排 Happy/Paws 用户可见功能从开发到 PR 的交付流程。用于实现或修改 PC Web、移动端或跨端功能，并需要独立验收、按需一对一 E2E、交互评审、移动端验收视频或 PR 前后对比时；也用于用户说‘视觉稿流程’或要求把功能完整验收到可提 PR。"
---
...
```text
开发 → 独立验收 → 按需一对一 E2E → 交互评审 → 按需视频 → PR 前后对比
```
...
| Web E2E | `web-e2e` | Case 对应关系、结果、最短复跑入口、产物 |
...
- PC/Web 默认不强制视频；用户明确要求，或截图无法证明时序时再录。
- 视频只录已经通过的 E2E/验收路径，不用失败录像充当结果。
- 移动端 Case 不需要自动化 E2E 时，录制同一个最小手工验收路径，并保留 `E2E: not-required` 的理由。
- 输出稳定 MP4 绝对路径到终端；运行时提供 Happy/Paws 文件发送能力时，再发送同一文件。没有发送能力时如实报告 `local-ready`，不宣称手机已收到。
...
3. PR 同时列出独立验收、E2E 是否需要及结果、交互评审、视频路径或稳定链接、未覆盖项。
...
- 最终回复先给 PR/交付结果，再列 Case 结果、E2E、视频路径和未完成项；不把 CI 等待时间隐藏在“创建 PR”里。
