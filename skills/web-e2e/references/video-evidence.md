...
负责把已经通过的 Web E2E Case 录成可复核 MP4，并在运行时能力允许时直接发送截图与视频。原生移动端录屏和 PR 编排仍由对应流程负责。
...
在终端输出绝对路径，同时完成当前运行时支持的媒体交付：

- 存在 `mcp__happy__send_image` 时，用它发送需要用户复核的 PNG / JPEG；
- 存在 `mcp__happy__send_file` 时，用它发送最终 MP4，并使用正确的 `video/*` MIME type；
- 媒体工具返回错误时可以在确认原因后重试，但不能把本地路径或一次失败调用描述成已发送；
- 没有发送能力或发送失败时保持 `local-ready` 或 `blocked`，并把原始错误简明报告给用户。

最终返回中增加 `delivery: sent | local-ready | blocked`。本地绝对路径只是终端产物入口，不是 Happy 手机端、其他设备或 PR 中的可见交付。
...
- [ ] 稳定绝对路径已输出给调用方。
- [ ] 运行时存在 Happy/Paws 媒体发送能力时，截图与 MP4 的发送调用均成功；失败已如实标记为 `local-ready` 或 `blocked`。
