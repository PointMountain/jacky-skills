...
```bash
m3u8-dl "<m3u8_url>" \
  -o "$HOME/Downloads/m3u8/<filename>.mp4" \
  -c 8
```
...
| `--referer` | `-r` | Referer 请求头 | `-r "https://example.com/"` |
...
m3u8-dl "https://example.com/video.m3u8" \
  -o "$HOME/Downloads/m3u8/$(date +%Y%m%d_%H%M%S).mp4" \
  -c 8
```
...
```bash
m3u8-dl "https://example.com/video.m3u8" \
  -o "$HOME/Downloads/m3u8/my_video.mp4"
```
...
```bash
m3u8-dl "https://example.com/video.m3u8" \
  -o "$HOME/Downloads/m3u8/preview.mp4" \
  -d 5
```
...
m3u8-dl "https://other-site.com/video.m3u8" \
  -o "$HOME/Downloads/m3u8/video.mp4" \
