...
| 截图目录 | 直接读 PNG/JPG |
| 页面 / CodePen / 本地 HTML | 通过 `browser-control → ego-ops → ego-browser` 打开并截图（关键时刻各截一张），同时读源码（源码能直接看到机制，优先级最高） |
...
| **元素身份与连续轨迹** | 堆叠展开、共享元素、卡片抽离等交互不能只比起末态：追踪**同一个元素**是否在至少三个中间帧连续经过不同位置。若实现切换两棵节点树再淡入淡出，即使帧率和起末态正确，也不是可见的抽离轨迹。堆叠态还要确认至少三条后层边缘在目标显示尺寸下清楚可见 |
...
2. 经 `browser-control → ego-ops → ego-browser` 打开，在与源视频相同的关键时刻截图
...
- 堆叠展开/共享元素类闭环已追踪同一元素至少三个中间位置，并在目标显示尺寸确认至少三条后层边缘可见；没有用帧率、编码或两张起末截图替代连续轨迹验收（见 L8）
